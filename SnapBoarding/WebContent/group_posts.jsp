	...
	<div class="row-fluid">
		<div class="box span12">
			...
			<div class="box-header">
				<h2>
					<i class="halflings-icon bullhorn"></i><span class="break"></span>GROUP-
					<c:out value="${grpName}"></c:out>
				</h2>
				<!--  id sent from server -->
				<div class="box-icon">
			...
			<div class="pagination pagination-centered">
				<ul>
					<c:if test="${currentPage != 1}">
						<!-- current page value -->
						<li><a href="empmsgfromgrp.htm?currentPage=${currentPage - 1}">Prev</a></li>
					</c:if>
					<!--
					...
					<c:forEach begin="1" end="${noOfPages}" var="i">
						<!-- divide in controller and send no of pages -->
						<c:choose>
							<c:when test="${currentPage eq i}">
								<li class="active"><a href="#">{i}</a></li>
							</c:when>
							<c:otherwise>
								<li><a href="empmsgfromgrp.htm?currentPage=${i}">${i}</a></li>
								<!-- send request to the controller-->
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<c:if test="${currentPage lt noOfPages}">
						<li><a href="empmsgfromgrp.htm?currentPage=${currentPage + 1}">Next</a></li>
					</c:if>
				</ul>
			</div>
		</div>
